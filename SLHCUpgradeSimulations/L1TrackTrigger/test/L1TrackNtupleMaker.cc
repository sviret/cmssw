  ...
  config(iConfig)
{
  ...
  NTrkPar              = iConfig.getParameter< int >("NTrkPar");
...
void L1TrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (DoPixelTrack) iEvent.getByLabel("L1PixelTrackFit", "Level1PixelTracks", TTPixelTrackHandle);
  ...
  if (SaveAllTracks) {
    ...
    for ( iterL1Track = TTTrackHandle->begin(); iterL1Track != TTTrackHandle->end(); iterL1Track++ ) {
      ...
      float tmp_trk_pt   = iterL1Track->getMomentum(NTrkPar).perp();
      float tmp_trk_eta  = iterL1Track->getMomentum(NTrkPar).eta();
      float tmp_trk_phi  = iterL1Track->getMomentum(NTrkPar).phi();
      float tmp_trk_z0   = iterL1Track->getPOCA(NTrkPar).z(); //cm
      float tmp_trk_chi2 = iterL1Track->getChi2(NTrkPar);
      float tmp_trk_consistency = iterL1Track->getStubPtConsistency(NTrkPar);
  ...
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
	...
	if (DebugMode) {
	  ...
	  cout << "   ... matched L1 track has pt = " << matchedTracks.at(it)->getMomentum(NTrkPar).perp() 
	       << " eta = " << matchedTracks.at(it)->getMomentum(NTrkPar).eta()
	       << " phi = " << matchedTracks.at(it)->getMomentum(NTrkPar).phi()
	       << " chi2 = " << matchedTracks.at(it)->getChi2() 
	       << " consistency = " << matchedTracks.at(it)->getStubPtConsistency() 
	       << " z0 = " << matchedTracks.at(it)->getPOCA(NTrkPar).z() 
    ...
    if (nMatch > 0) {
      tmp_matchtrk_pt   = matchedTracks.at(i_track)->getMomentum(NTrkPar).perp();
      tmp_matchtrk_eta  = matchedTracks.at(i_track)->getMomentum(NTrkPar).eta();
      tmp_matchtrk_phi  = matchedTracks.at(i_track)->getMomentum(NTrkPar).phi();
      tmp_matchtrk_z0   = matchedTracks.at(i_track)->getPOCA(NTrkPar).z();
      tmp_matchtrk_chi2 = matchedTracks.at(i_track)->getChi2(NTrkPar);
      tmp_matchtrk_consistency = matchedTracks.at(i_track)->getStubPtConsistency(NTrkPar);
