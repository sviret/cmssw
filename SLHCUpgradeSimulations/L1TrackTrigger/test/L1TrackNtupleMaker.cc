...
private:
  ...
  edm::EDGetTokenT< TTTrackAssociationMap< Ref_Phase2TrackerDigi_ > > MCTruthTrackInputTag;
  edm::EDGetTokenT< std::vector< TTTrack< Ref_Phase2TrackerDigi_ > > > L1TrackInputTag;
  edm::EDGetTokenT< TTClusterAssociationMap< Ref_Phase2TrackerDigi_ > > TTClusAInputTag;
  //  edm::EDGetTokenT< edmNew::DetSetVector< TTStub< Ref_Phase2TrackerDigi_ > > > stubsToken_;
  edm::EDGetTokenT< TTStubAssociationMap< Ref_Phase2TrackerDigi_ > > TTStubAInputTag;
  ...
  config(iConfig)
{
  ...
  L1TrackInputTag      = consumes< std::vector< TTTrack< Ref_Phase2TrackerDigi_  > > >(iConfig.getParameter< edm::InputTag >( "L1TrackInputTag" ));
  MCTruthTrackInputTag = consumes< TTTrackAssociationMap< Ref_Phase2TrackerDigi_  > >(iConfig.getParameter< edm::InputTag >( "MCTruthTrackInputTag" ));

  TTClusAInputTag = consumes< TTClusterAssociationMap< Ref_Phase2TrackerDigi_  > >(config.getParameter< edm::InputTag >( "TTClustersAssociators" ));
  TTStubAInputTag = consumes< TTStubAssociationMap< Ref_Phase2TrackerDigi_  > >(config.getParameter< edm::InputTag >( "TTStubsAssociators" ));
  TPInputTag      = consumes< TrackingParticleCollection >(config.getParameter< edm::InputTag >( "TrkParticles" )); 
}
...
void L1TrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken(L1TrackInputTag, TTTrackHandle);
  ...
  iEvent.getByToken(MCTruthTrackInputTag, MCTruthTTTrackHandle);
  edm::Handle< TTClusterAssociationMap< Ref_Phase2TrackerDigi_ > > MCTruthTTClusterHandle;
  iEvent.getByToken(TTClusAInputTag, MCTruthTTClusterHandle);
  edm::Handle< TTStubAssociationMap< Ref_Phase2TrackerDigi_ > > MCTruthTTStubHandle;
  iEvent.getByToken(TTStubAInputTag, MCTruthTTStubHandle);

  // tracking particles
  edm::Handle< std::vector< TrackingParticle > > TrackingParticleHandle;
  iEvent.getByToken(TPInputTag, TrackingParticleHandle);
  ...
  double mMagneticFieldStrength = 3.8;
  //if (SaveStubs) {  
  ...
  if (SaveStubs) {
    ...
    for (auto gd=tGeom->dets().begin(); gd != tGeom->dets().end(); gd++) 
    {
      ...
      {
	edm::Ref< edmNew::DetSetVector< TTStub< Ref_Phase2TrackerDigi_  > >, TTStub< Ref_Phase2TrackerDigi_  > > stub = edmNew::makeRefTo( TTStubHandle, stubIter );
	      
	MeasurementPoint coords = stub->getClusterRef(0)->findAverageLocalCoordinates();

	LocalPoint clustlp = topol->localPosition(coords);
	GlobalPoint posStub  =  theGeomDet->surface().toGlobal(clustlp);
	
	bool isPS = (tGeom->getDetectorType(stub->getDetId())==TrackerGeometry::ModuleType::Ph2PSP);
	bool isBarrel = detid.subdetId()==StripSubdetector::TOB;

	int layer = -1;
	if (isBarrel) layer = static_cast<int>(tTopo->layer(detid));
	else layer = static_cast<int>(tTopo->tidWheel(detid))*static_cast<int>(tTopo->side(detid));
	...
	//
	// SV 07/07/16: this method has to be implemented
	//
  ...
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
    ...
    {
	edm::Ref< edmNew::DetSetVector< TTStub< Ref_Phase2TrackerDigi_ > >, TTStub< Ref_Phase2TrackerDigi_ > > tempStubRef = theStubRefs.at(is);
	...
	for (auto gd=tGeom->dets().begin(); gd != tGeom->dets().end(); gd++) 
	{
	    ...
	    if(tTopo->stack(detidg)!=stackDetid) continue; 
	...
	if (isBarrel) layer = static_cast<int>(tTopo->layer(detid))-1;
	else layer = static_cast<int>(tTopo->tidWheel(detid))+5;
