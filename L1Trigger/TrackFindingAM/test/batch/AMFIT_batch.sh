...
do   
    ...
    OUTB=`echo $l | cut -d. -f1 | cut -d_ -f1`

    OUTF=${OUTM}"_and_FIT.root"
    OUTE=${OUTB}"_extr.root"
    OUTD=${OUTB}"_off.root"

    FIT_FILE_I=${OUTDIR_GRID}/$OUTF

    dealI=`gfal-ls $FIT_FILE_I | wc -l`
#    dealT=`lcg-ls $FIT_FILE_T | wc -l`
    ...
    if [ ${4} == "BATCH" ]; then	
	bsub -R "pool>10000" -q $BQUEUE -e /dev/null -o /tmp/${LOGNAME}_out.txt run_${nsj}_${MATTER}.sh
