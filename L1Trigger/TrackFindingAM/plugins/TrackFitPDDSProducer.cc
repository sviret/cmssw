...
#ifndef TRACK_FITTER_AM_PDDS_H
#define TRACK_FITTER_AM_PDDS_H
...
class TrackFitPDDSProducer : public edm::EDProducer
{
  public:
    /// Constructor
    explicit TrackFitPDDSProducer( const edm::ParameterSet& iConfig );

    /// Destructor;
    ~TrackFitPDDSProducer();
  ...
  std::shared_ptr<PDDS> combinationBuilder_;
  int m_max_roads;
  int m_max_fits;
...
/// Constructors
TrackFitPDDSProducer::TrackFitPDDSProducer( const edm::ParameterSet& iConfig )
{
  ...
  combinationBuilder_ = std::make_shared<PDDS>();
  if (advancedCombinationBuilder) combinationBuilder_->SetFive(true);
  else combinationBuilder_->SetFive(false);
  m_max_roads              = iConfig.getParameter< int >( "maxRoadsperTower" );
  m_max_fits                = iConfig.getParameter< int >( "maxFitsperTower" );

  produces< std::vector< TTTrack< Ref_PixelDigi_ > > >( TTTrackOutputTag );
}

/// Destructor
TrackFitPDDSProducer::~TrackFitPDDSProducer() {}

/// Begin run
void TrackFitPDDSProducer::beginRun( const edm::Run& run, const edm::EventSetup& iSetup )
{
...
/// End run
void TrackFitPDDSProducer::endRun( const edm::Run& run, const edm::EventSetup& iSetup ) {}

/// Implement the producer
void TrackFitPDDSProducer::produce( edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  /// Prepare output
  ...
  for(int i=0;i<48;i++) comb_mult[i]=0;
  ...
  if ( TTPatternHandle->size() > 0 ) {
	...
	m_hits.push_back(h);
      ...
      //determine the kind of road loaded
      ...
      std::vector<StubsCombination> stubsCombinations = combinationBuilder_->combine(road);
      ...
      if (static_cast<int>(seedSector)<48){
	++comb_mult[static_cast<int>(seedSector)%100];
	if (comb_mult[static_cast<int>(seedSector)%100]>m_max_fits && m_max_fits>0) continue;
      }

      for (unsigned int comb = 0; comb < stubsCombinations.size(); ++comb) {
        tempVec.clear();
        StubsCombination stubsCombination=stubsCombinations[comb];
...
DEFINE_FWK_MODULE(TrackFitPDDSProducer);
